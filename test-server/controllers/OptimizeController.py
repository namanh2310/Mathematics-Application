          ...
          def goldenSectionSearchFunction(xl, xu, d_constant, x1, x2, f1, f2, es, type):
            ...
            while (ea > es):
              if i == 0:
                ...
                obj.append(element)
                comp = '>' if f1 > f2 else '<'
                step.append(f"Firstly, we calculate \\(d = R(x_u - x_l) = (\\frac{{\sqrt{5} - 1}}{{2}}) ({element['xu']} - {element['xl']}) = {element['d']}\\)")
                step.append(f"Besides, \\(x_1 = x_l + d = {element['xl']} + {element['d']} = {element['x1']}\\)")
                step.append(f"And, \\(x_2 = x_u - d = {element['xu']} - {element['d']} = {element['x2']}\\)")
                step.append(f"Now, check \\(f(x_1) = {element['f1']} {comp} f(x_2) = {element['f2']}\\)")
                step.append(f"Therefore, \\(x_l = x_{{opt}} = x_2 = {element['x2']}\\)")
                step.append(f"Error will be, \\(e_a = (1 - R) * \\frac{{interval}}{{x_{{opt}}}} \\cdot 100\\)")
                step.append(f"\\(= (1 - \\frac{{\\sqrt{5} - 1}}{{2}})\\frac{{{element['xu']} - {element['xl']}}}{{{element['x2']}}} \\cdot 100 = {element['ea']}\\)")
              else:
    ...
    def newtonMethod():
      if request.method == "POST":
        ...
        try:
          ...
          def nDerivative(n):
            x = Symbol("x")
            return latex((diff(functionInput(x), x, n)))
          ...
          def newtonMethodFunction(i, x0, originFunc, firstDeriFunc, secondDeriFunc, x_next, ea, es):
            while (i < 100):
              if i == 0:
                ...
                x_0_next = x0 - firstDeriFunc / secondDeriFunc
                print("nbnnnnn", x_0_next)
                obj.append(element)
                step.append(f"\\(f(x) = {function}\\)")
                step.append(f"\\(\\rightarrow f({x0}) = {element['fx']}\\)")
                step.append(f"\\(f'(x) = {nDerivative(1)}\\)")
                step.append(f"\\(\\rightarrow f({x0}) = {element['f_1st']}\\)")
                step.append(f"\\(f'(x) = {nDerivative(2)}\\)")
                step.append(f"\\(\\rightarrow f({x0}) = {element['f_2nd']}\\)")
                step.append(f"Next, we apply a formula: \\(x_{1} = x_i - \\frac{{f'(x_0)}}{{f''(x_0)}}\\)")
                step.append(f"\\(x_1 = {element['x0']} - \\frac{{{element['f_1st']}}}{{{element['f_2nd']}}} = {x_0_next}\\)")
          ...
          while (i < 14):
            if i == 0:
                ...
                f_obj.append({'a_iteration': i+1,
                              'b_x_0': '{:.4f}'.format(x_0),
                              'c_f_0': '{:.4f}'.format(funct(x_0)),
                              'd_x_1': '{:.4f}'.format(x_1),
                              'e_f_1': '{:.4f}'.format(funct(x_1)),
                              'f_x_2': '{:.4f}'.format(x_2),
                              'g_f_2': '{:.4f}'.format(funct(x_2)),
                              'h_x_3': '{:.4f}'.format(x_3),
                              'i_f_3': '{:.4f}'.format(funct(x_3))})
            else:
                x_0 = x_1
                x_1 = x_2
                x_2 = x_3
                ...
                if j == 0:
                  ...
                  if funct(x_1) > funct(x_3):
                    ...
                    if x_1 < x_3:
                      ...
                      x_2 = x_1
                      x_1 = x_3
                      step.append(f"Next, we have \\(x_1 >= x_3\\). So \\(x_2 = x_1 = {round(x_1, 4)} ; x_1 = x_3 = {round(x_3, 4)} \\)")
